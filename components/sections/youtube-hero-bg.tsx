...
function loadYouTubeApi(): Promise<typeof window.YT> {
  ...
  ytApiPromise = new Promise((resolve, reject) => {
    // Chain any pre-existing onYouTubeIframeAPIReady handler so we don't
    // clobber third-party code.
 ...
 * The parent layers (shader grid, machine collage, gradients) carry the
 * hero unchanged — the viewer never sees a broken embed.
 *
  ...
  useEffect(() => {
    ...
    loadYouTubeApi()
      .then((YT) => {
        ...
        // Watchdog: if we never reach PLAYING within failTimeoutMs, the embed
        // is gated (bot wall, Shorts block, region restriction). Bail out.
        watchdog = setTimeout(() => {
        ...
        playerRef.current = new YT.Player(target, {
          videoId: id,
          // Use youtube-nocookie host for privacy-enhanced mode.
          host: 'https://www.youtube-nocookie.com',
          playerVars: {
            autoplay: 1,
            mute: 1,
            loop: 1,
            playlist: id, // loop=1 needs the same id passed as playlist
            controls: 0,
            modestbranding: 1,
            rel: 0,
            iv_load_policy: 3,
          ...
          events: {
            onReady: (e) => {
              // Force-trigger play; some browsers throttle the playerVars hint.
              try {
                e.target.mute();
                e.target.playVideo();
              } catch {
  ...
  return (
    <div
      ...
      style={{
        ...
        // object-fit: cover equivalent applied to the iframe YT injects.
        // The child <iframe> picks up these CSS variables via the global
        // selector below.
      }}
      title={title}
    >
      {/* Inject sizing styles for the YT-injected iframe — global because
          we don't control the iframe element's className. Scoped to this
          component via the data attribute on the mount node. */}
      <style jsx>{`
...
// Full surface area is at https://developers.google.com/youtube/iframe_api_reference
// ────────────────────────────────────────────────────────────────────────
declare global {
  interface Window {
    YT?: typeof YT;
